...
#%% import data
X_train,y_train=makedataliblinear('train.new')
y_train=2*y_train-1
ss=np.sum(X_train,0)
idf=IDF(X_train)
...
X_test,y_test=makedataliblinear('test.new')
y_test=2*y_test-1
X_test=np.append(X_test,np.zeros([np.size(X_test,0),np.size(X_train,1)-np.size(X_test,1)]),1)
X_test=idf*TF(X_test)
X_train=X_train[:,ss>n_ignore]
X_test=X_test[:,ss>n_ignore]


X_eval,y_eval=makedataliblinear('eval.new')
X_eval=np.append(X_eval,np.zeros([np.size(X_eval,0),len(ss)-np.size(X_eval,1)]),1)
X_eval=idf*TF(X_eval)
X_eval=X_eval[:,ss>n_ignore]
